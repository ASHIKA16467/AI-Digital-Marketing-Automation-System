...
function generateContent() {
    ...
    if (platform === "twitter") {
        prompt = `Write a short promotional tweet (strictly under 180 characters) for "${companyName}" and its project "${projectName}". Use the tagline "${tagline}" and a keyword from: ${keywords}. Do not mention Bangalore. Reply only with the tweet text. No heading or explanation.`;
    } else {
        prompt = `
Write a digital marketing content strategy for a company named "${companyName}" with a project called "${projectName}".
Use the tagline "${tagline}" and focus on the keywords: ${keywords}. 

Incorporate these additional suggestions:
${allSuggestions}

The content should include:
- A short intro (50-70 words) about the company based on the tagline.
- Key benefits of the service or product.
- How the company stands out in its domain.
- SEO-friendly paragraph using the provided keywords naturally.
- A catchy call-to-action at the end.

Write in a friendly, marketing-oriented tone that engages users online.
`;
    }
    ...
    .catch(error => {
        document.getElementById('generatedContent').innerHTML = "❌ Error: " + error;
    });
}
